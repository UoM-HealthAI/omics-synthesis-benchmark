...
class cscGAN:
    ...
    def __init__(self, train_files, valid_files, genes_no, clusters_no,
        ...
        print("Initializing cscGAN model...")
        """
        ...
        print("Setting up training input pipeline...")
        self.train_cells, self.train_cells_clusters = self.make_input_fn(
            self.train_files)
        print("Training input pipeline created")

        # prepare input pipeline for validation
        print("Setting up validation input pipeline...")
        self.test_cells, self.test_cells_clusters = self.make_input_fn(
            self.valid_files)
        print("Validation input pipeline created")

        # module parameters
        print("Setting up module parameters...")
        self.generator = None
    ...
    def make_input_fn(self, file_paths, epochs=None):
        ...
        # Use TensorFlow 2.x compatible data API
        print(f"    Loading TFRecord files: {file_paths}")
        dataset = tf.data.TFRecordDataset(file_paths, compression_type="GZIP")
        
        def parse_function(example_proto):
            return tf.parse_single_example(example_proto, feature_map)
        
        print("    Parsing and batching dataset...")
        ...
        batched_features = iterator.get_next()
        ...
        cluster = tf.squeeze(tf.cast(batched_features['cluster_int'], tf.int32))
    ...
    def build_model(self):
        ...
        with tf.name_scope('critic_loss'):
            ...
            differences = self.generator.fake_outputs - self.train_cells
    ...
    def training(self, exp_folder, clusters_ratios, checkpoint=None,
        ...
        print(f"Starting cscGAN training in folder: {exp_folder}")
        """
        ...
        print("Creating TensorFlow Supervisor...")
        train_supervisor = tf.train.Supervisor(logdir=exp_folder,
                                               save_summaries_secs=0,
                                               saver=None)
        print("Supervisor created, starting managed session...")
        ...
        with train_supervisor.managed_session() as sess:
            print("Managed session started, beginning training...")
