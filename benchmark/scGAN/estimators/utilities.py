...
class Generator:
    ...
    def create_cond_generator(cls, z_input, batch_size, latent_dim,
        ...
        with tf.variable_scope(var_scope, reuse=reuse):
            print(f"    Creating generator with {len(gen_layers)} layers: {gen_layers}")
            for i_lay, size in enumerate(gen_layers):
                print(f"    Creating generator layer {i_lay + 1} with size {size}")
                with tf.variable_scope("generator_layers_" + str(i_lay + 1)):
                    z_input = tf.layers.dense(
                        z_input,
                        size,
                        kernel_initializer=tf.glorot_uniform_initializer(),
                        use_bias=False)
                    print(f"    Linear layer {i_lay + 1} created")
            ...
            print(f"    Generator layers created 175 ")
            with tf.variable_scope("generator_layers_" + 'output'):
                fake_outputs = tf.layers.dense(
                ...
                if output_lsn is not None:
                    ...
                    fake_outputs = tf.transpose(tf.transpose(fake_outputs) *
                                                scale_ls)
            print(f"    Generator output layer created")
