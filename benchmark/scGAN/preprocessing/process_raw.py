...
class GeneMatrix:
    ...
    def write(self):
        ...
        # Convert any byte keys to strings for JSON serialization
        def convert_keys_to_str(d):
            if isinstance(d, dict):
                return {str(k) if isinstance(k, bytes) else k: convert_keys_to_str(v) for k, v in d.items()}
        ...
        hparam['preprocessed'] = {
            ...
            'train_cells_per_cluster': convert_keys_to_str(self.train_cells_per_cluster),
            'valid_cells_per_cluster': convert_keys_to_str(self.valid_cells_per_cluster),
            'test_cells_per_cluster': convert_keys_to_str(self.test_cells_per_cluster),
            'split_seed': self.split_seed,
            'scale': self.scale,
            'clusters_no': self.clusters_no,
            'clusters_ratios': convert_keys_to_str(self.clusters_ratios)}
